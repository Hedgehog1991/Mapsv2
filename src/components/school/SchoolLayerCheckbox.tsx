...
import { kommuneLayer } from "../kommune/kommuneLayer";
...
function schoolStyle(f: FeatureLike) {
  ...
  return new Style({
    image: new Circle({
      ...
      fill: new Fill({
        color: school.eierforhold === "Offentlig" ? "brown" : "orange",
      }),
...
function activeSchoolStyle(f: FeatureLike, resolution: number) {
  ...
  return new Style({
    image: new Circle({
      ...
      fill: new Fill({
        color: school.eierforhold == "Offentlig" ? "orange" : "brown",
      }),
      radius: 3 + school.antall_elever / 150,
    }),
    text:
